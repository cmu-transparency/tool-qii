...
def get_histogram_bins(X, cols, num_bins=40):
	col = cols[0]
	column_values = X[:, col]
...
def yield_increasing_bins(X, target_class_X, binned_indices, cols, bin):
	""" Randomly intervene on a a set of columns of target_class_X. """
	p = target_class_X.shape[0]
	target_int = numpy.array(target_class_X)
	indices_list = [index for index, value in enumerate(binned_indices) if value == bin]
	n = len(indices_list)
	if n == 0:
		return None
	else:
...
def get_feature_variations(features_list, dataset, cls, X, target_class_X, class_name):
	average_local_inf_class = pd.DataFrame()
	bins = 40
	iters = 10
	f_columns = dataset.num_data.columns
	sup_ind = dataset.sup_ind
