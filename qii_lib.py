...
def random_intervene(X, cols):
    ...
    n = X.shape[0]
    order = numpy.random.permutation(list(range(n)))
    X_int = numpy.array(X)
    for c in cols:
...
def shapley_mask(dataset,  X_test):
    ...
    counterfactuals_w = dict([(sf, numpy.zeros((p_samples*s_samples, X_test.shape[1])))
        for sf in list(dataset.sup_ind.keys())])
    ...
    for sample in range(0, s_samples):
        ...
        for i in range(0, len(super_indices)):
            ...
            for k in (ls_m_si):
                counterfactuals_wo[si][list(range(sample*p_samples, (sample+1)*p_samples)), k] = 1
            ...
            for k in (ls_m_si+ls_si):
                counterfactuals_w[si][list(range(sample*p_samples, (sample+1)*p_samples)), k] = 1
...
def shapley_influence_cached(dataset, cls, x_individual, X_test):
    ...
    s_samples = 1
    num_iterations = 600
    features = list(dataset.sup_ind.keys())
...
def banzhaf_influence(dataset, cls, x_individual, X_test):
    ...
    for sample in range(0, s_samples):
        ...
        S = [super_indices[i] for i in range(0, len(super_indices)) if r[i] > 0.5]
...
def analyze_outliers(counterfactuals, out_cls, cls):
    ...
    new_outlier_fracs = {}
    qii = {}
    for sf, pairs in counterfactuals.items():
        X = pairs[0]
        X_cf = pairs[1]
